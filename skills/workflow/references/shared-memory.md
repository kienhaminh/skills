...
Store only `fact`, `decision`, `risk`, `question`, `learning`, or `handoff` entries. Keep summaries under 500 characters; reference artifacts by SHA-256. Never store chain-of-thought, transcripts, source/test payloads, credentials, environment values, or state derivable from graph/runtime.
