...
class SharedMemoryTests(unittest.TestCase):
    ...
    def entry(self, *, entry_id: str = "M-F-1", kind: str = "fact", pivotal: bool = False, summary: str = "Focused verification passed.") -> dict:
        evidence = self.workflow / "evidence" / "memory-check.txt"
        evidence.parent.mkdir(exist_ok=True)
        evidence.write_text("focused verification passed\n", encoding="utf-8")
        ...
        return {
            ...
            "requirement_ids": ["R5"],
            "relevant_nodes": ["F"],
            "artifact_refs": [{"path": "evidence/memory-check.txt", "digest": digest}] if kind != "question" else [],
            "pivotal": pivotal,
            "supersedes": None,
        }
