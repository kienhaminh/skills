...
function analyzeGraph(graph) {
  ...
  const phase = errors.length
    ...
    : allExpanded && allComplete && evidenceReady && lifecycleComplete
      ? "complete"
      : allExpanded && coverageExact && intentReady
        ? "executable"
        : "draft";
...
function renderHeader() {
  ...
  elements.goalBinding.textContent = `Lifecycle: ${lifecycle.status || "unknown"} · Intent: ${intentLabel} · Evidence: ${verification.outcome || "missing"}`;
  const cost = asObject(graph.cost_plan);
  elements.planSummary.textContent = cost.selected
    ? `${humanizeId(cost.selected)} · ${cost.assessment || "unreviewed"} · ${formatTokens(cost.token_budget)} tokens`
    : "Execution plan supplied by caller or not required.";
...
function nodeMatchesFilters(node) {
  ...
  const searchable = [node.id, node.title, node.kind, node.status, node.isolation, runtime.agent, runtime.model, runtime.reasoning_effort, ...asArray(node.methods), ...asArray(node.skills), ...asArray(node.covers)]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
...
function createNodeCard(node) {
  ...
  card.append(createElement("div", "node-meta", `agent: ${runtime.agent || "unassigned"} · skills: ${asArray(node.skills).join(", ") || "none"}`));
