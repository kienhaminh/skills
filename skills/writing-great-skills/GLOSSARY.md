...
A skill whose invocation is enabled, normally by omitting `disable-model-invocation`. Its
**description** is visible to the agent as a trigger, so the agent can invoke it autonomously and the
human can still type its name. It pays permanent **context load** in exchange for discoverability and
can be reached by other skills. Pick model invocation when autonomous or cross-skill reach is worth
that load.
...
A skill with `disable-model-invocation: true`. Its short human-facing **description** remains in the
artifact and UI metadata but is withheld as an autonomous model trigger. Only the human can invoke
the skill, so it trades agent discoverability for zero trigger **context load**.

_Avoid_: procedure, workflow, command

### Description

A one-line summary in skill metadata. For a **model-invoked** skill it is the machine-readable trigger
and top-level **context pointer**. For a **user-invoked** skill it is human-facing metadata and must
not contain trigger lists. Invocation policy, rather than field presence alone, decides whether the
description enters model context.
...
The cost a **model-invoked** skill imposes on the agent's context window—its trigger description,
spending both tokens and attention. **User-invoked** skills escape this trigger load because invocation
policy withholds their human-facing description from autonomous discovery.
...
A **user-invoked** skill whose job is to point at other user-invoked skills—naming each and when the
human should reach for it—so the human remembers one entrypoint instead of many. It can recommend but
cannot autonomously fire skills whose invocation policy is user-only. The cure for **cognitive load**
when user-invoked skills multiply.
