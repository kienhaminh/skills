...
EVIDENCE_RUNNER = "workflow-evidence-runner-v1"
...
def validate_entry(
    ...
    *,
    ...
    if evidence_state == "verified" and (owner == "coordinator" or nodes.get(owner, {}).get("kind") != "verify"):
        add_error(errors, f"{where}.owner_node", "verified memory must be authored by a verify node")
        ...
        if resolved is None:
            ...
            if not resolved.is_file():
                ...
                try:
                    ...
                    envelope = None
                if not isinstance(envelope, dict) or envelope.get("runner") != EVIDENCE_RUNNER or envelope.get("passed") is not True:
                    add_error(errors, ref_where, "verified memory requires a passing workflow evidence-runner attestation")
